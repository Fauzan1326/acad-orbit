...
export default function Analytics() {
  ...
  const conflictTypes = ['Teacher', 'Room', 'Semester', 'RoomRule'].map(type => ({
    name: type, value: clashes.filter(c => c.type === type).length,
  })).filter(c => c.value > 0);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="glass-card p-5">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={teacherWorkload}>
              ...
              <Tooltip contentStyle={{ background: 'hsl(222, 41%, 10%)', border: '1px solid hsl(222, 30%, 18%)', borderRadius: 8 }} labelStyle={{ color: 'hsl(210, 40%, 93%)' }} />
        ...
        <div className="glass-card p-5">
          ...
          {conflictTypes.length === 0 ? (
            <div className="h-[250px] flex items-center justify-center text-muted-foreground text-sm">No conflicts detected ✓</div>
          ) : (
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie data={conflictTypes} cx="50%" cy="50%" innerRadius={50} outerRadius={90} dataKey="value" label={({ name, value }) => `${name}: ${value}`}>
                  {conflictTypes.map((_, i) => <Cell key={i} fill={CHART_COLORS[i % CHART_COLORS.length]} />)}
                </Pie>
                <Tooltip contentStyle={{ background: 'hsl(222, 41%, 10%)', border: '1px solid hsl(222, 30%, 18%)', borderRadius: 8 }} />
              </PieChart>
            </ResponsiveContainer>
          )}
        </div>
